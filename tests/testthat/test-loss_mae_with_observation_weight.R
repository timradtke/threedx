...
test_that("equal to `sqrt(mean(AE^2))` when all observations have values less than the absolute error", {
  y <- c(-5, 1, 10, -49305)
  y_hat <- c(1, 4, -485, -50)
  expect_identical(
    loss_mae_with_observation_weight(y_hat = y_hat, y = y),
    sqrt(mean(abs(y - y_hat) * abs(y - y_hat)))
  )
})
