...
expect_weights_seasonal_decay <- function(x, len, alpha, period_length) {
  expect_numeric(
    x = x,
    len = len,
    lower = 0,
  ...
  expect_true(length(unique(x)) <= ceiling(len / period_length))
...
test_that("returns 'random walk' weights when `alpha=1`", {
  ...
  for (i in seq_len(nrow(inputs))) {
    ...
    # random walk means that the most recent `period_length` weights are equal
    # and all others are 0
    last_period_start <- pmax(1, inputs$n[i] - inputs$period_length[i] + 1)
