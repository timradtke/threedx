...
expect_weights_seasonal <- function(x, len, alpha, season_length) {
  expect_numeric(
    x = x,
    len = len,
    lower = 0,
  ...
  expect_true(length(unique(x)) <= season_length / 2 + 1)
  
  if (len >= season_length) {
    expect_equal(x[len - season_length + 1], max(x))
    
    expect_equal(
      x[pmax(1, len - season_length + 1):len] /
        sum(x[pmax(1, len - season_length + 1):len]),
      weights_seasonal(
        alpha_seasonal = alpha, n = season_length, season_length = season_length
      )
    )
  } else {
...
test_that("returns random walk weights when `alpha = 1` and `n < season_length", {
  inputs <- expand.grid(
    n = c(1, 2, 6, 7, 8, 10, 11, 12, 13, 14, 23, 24, 25, 100, 1000),
    season_length = c(1, 2, 3, 7, 12)
  )
  inputs <- inputs[inputs$n < inputs$season_length, ]
...
test_that("returns seasonal mean weights when `alpha = 1` and `n >= season_length", {
  inputs <- expand.grid(
    n = c(1, 2, 6, 7, 8, 10, 11, 12, 13, 14, 23, 24, 25, 100, 1000),
    season_length = c(1, 2, 3, 7, 12)
  )
  ...
  for (i in seq_len(nrow(inputs))) {
    ...
    expected_idx <- rev(seq_len(inputs$n[i]) %% inputs$season_length[i] == 0)
